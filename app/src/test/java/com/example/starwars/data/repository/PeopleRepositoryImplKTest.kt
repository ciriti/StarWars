...
class PeopleRepositoryImplKTest {

    private val mockApiService: StarWarsApiClient = mockk()
    private val personDao: PersonDao = mockk()
    private val paginatedPersonDao: PaginatedPersonDao = mockk()
    private val peopleRepository: PeopleRepositoryK = PeopleRepositoryKtorImpl(
        mockApiService,
        personDao,
        paginatedPersonDao,
        isExpired = { _, _ -> true }
    ...
    lateinit var personDto: PersonDtoK
    ...
    fun `getPersonById should return person when API call is successful`() = runBlocking {
        // Arrange
        coEvery { mockApiService.getPersonById(any()) } returns Result.success(personDto)
    ...
    fun `getPersonById should return an error when API call fails`() = runBlocking {
        // Arrange
        coEvery { mockApiService.getPersonById(personId) } returns Result.failure(RuntimeException("Error"))
    ...
    fun `getPeople should return paginated response when API call is successful`() = runBlocking {
        ...
        coEvery { mockApiService.getPeople(1) } returns Result.success(paginatedResponse)
    ...
    fun `getPeople should return an error when API call fails`() = runBlocking {
        // Arrange
        coEvery { mockApiService.getPeople(1) } returns Result.failure(RuntimeException("Error"))
    ...
    fun `getPeople should invoke paginatedPersonDao when cache is not expired`() = runBlocking {
        ...
        val peopleRepository: PeopleRepositoryK = PeopleRepositoryKtorImpl(
            mockApiService,
            personDao,
            paginatedPersonDao,
            isExpired = { _, _ -> false }
